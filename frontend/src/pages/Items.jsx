...
export default function Items({ toast }) {
  ...
  const { isLoading: lootConfirming, isSuccess: lootSuccess } =
    useWaitForTransactionReceipt({ hash: lootTxHash });

  useEffect(() => {
    if (lootSuccess) { toast?.success("Loot drop requested! Waiting for VRF…"); refetchBalances(); }
  }, [lootSuccess]);
  ...
  const { isLoading: craftConfirming, isSuccess: craftSuccess } =
    useWaitForTransactionReceipt({ hash: craftTxHash });

  useEffect(() => {
    if (craftSuccess) { toast?.success(`${craftMeta?.name} crafted!`); refetchBalances(); setSelectedCraft(null); }
  }, [craftSuccess]);
    ...
    <div className="page">
      ...
      <div className="card section-gap">
        ...
        <p className="text-sm text-muted mt-1">
          Drop rates and crafting costs are controlled by DAO governance.
          Vote on proposals in the <a href="/governance">Governance</a> tab to change them.
        </p>
      </div>
      ...
      <div className="card section-gap">
        ...
        <p className="text-sm text-muted mt-1" style={{ marginBottom: "0.75rem" }}>
          Pay a small fee to trigger a provably random loot drop powered by Chainlink VRF.
          You will receive random in-game items based on the current drop rate.
        </p>
        {!isConnected ? (
          ...
          <div style={{ display: "flex", gap: "1rem", alignItems: "center", flexWrap: "wrap" }}>
            ...
            >
              {lootPending || lootConfirming ? (
                <><span className="spinner" style={{ width: 14, height: 14 }} /> {lootConfirming ? "Waiting for VRF…" : "Requesting…"}</>
      ...
      {/* ── Crafting ────────────────────────────────────────────────────── */}
      <div className="card section-gap">
        <div className="card-title" style={{ marginBottom: "0.75rem" }}>Crafting</div>
          ...
          <>
            ...
            {selectedCraft && (
              <div className="card" style={{ background: "var(--bg3)", marginBottom: "0.75rem" }}>
                <div className="card-title">Recipe: {craftMeta?.emoji} {craftMeta?.name}</div>
                {craftingCost ? (
                  <div className="mt-1">
                    {craftingCost[0].map((inputId, i) => {
                      ...
                      return (
                        <div key={i} className="stat-row">
                          <span className="label">{mat?.emoji} {mat?.name}</span>
                          <span className="value mono" style={{ color: have >= need ? "var(--success)" : "var(--danger)" }}>
                            {have} / {need}
                          </span>
                        </div>
                      );
                ...
                >
                  {craftPending || craftConfirming ? (
                    <><span className="spinner" style={{ width: 14, height: 14 }} /> {craftConfirming ? "Confirming…" : "Crafting…"}</>
      ...
      {/* ── Resources reference ──────────────────────────────────────────── */}
      <div className="card">
        <div className="card-title" style={{ marginBottom: "0.75rem" }}>Resource Guide</div>
        <table>
          <thead>
            <tr>
              <th>Item</th>
            ...
            {ITEM_IDS_LIST.map((id) => {
              ...
              return (
                <tr key={id}>
                  ...
                  <td><span className="badge" style={{ background: "var(--bg3)", color: "var(--text2)" }}>{meta.type}</span></td>
